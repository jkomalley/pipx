...
def run_pipx_command(args: argparse.Namespace, subparsers: Dict[str, argparse.ArgumentParser]) -> ExitCode:  # noqa: C901
    ...
    if args.command == "run":
        ...
        return commands.show(package, venv_dir)
    else:
...
def _add_show(subparsers: argparse._SubParsersAction, venv_completer: VenvCompleter, shared_parser: argparse.ArgumentParser) -> None:
    p = subparsers.add_parser(
        "show",
        help="Show information about a pipx installed package.",
        description="Show information about a pipx installed package.",
        parents=[shared_parser],
    )
    p.add_argument("package", help="Name of the package").completer = venv_completer
